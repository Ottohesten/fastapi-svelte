...
export function createColumns(userUpdateForm: SuperForm<Infer<typeof UserUpdateSchema>>, userAddRoleForm: SuperForm<Infer<typeof UserAddRoleSchema>>, roles: components['schemas']['RolePublic'][], availableScopes: string[] = []): ColumnDef<components['schemas']["UserWithPermissionsPublic"]>[] {
    return [
        {
            accessorKey: "email",
            header: "Email",
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return renderComponent(UserActions, { user, userUpdateForm, permissions: user, userAddRoleForm, roles, availableScopes });
            },
        }
    ];
}
