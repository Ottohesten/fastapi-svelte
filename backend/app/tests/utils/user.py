...
def authentication_token_from_email(client: TestClient, email: str, db: Session) -> dict[str, str]:
    ...
    """
    password = random_lower_string()
    user = db_crud.get_user_by_email(session=db, email=email)
    if not user:
        user_in_create = UserCreate(email=email, password=password)
        user = db_crud.create_user(session=db, user_create=user_in_create)
    else:
        user_in_update = UserUpdate(password=password)
        if not user.id:
            raise Exception("User id not set")
        user = db_crud.update_user(session=db, db_user=user, user_in=user_in_update)
