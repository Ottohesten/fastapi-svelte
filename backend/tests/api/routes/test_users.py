...
def test_retrieve_users(
    ...
    db_crud.create_user(session=db, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = db_crud.create_user(session=db, user_create=user_in2)
...
def test_delete_user_current_super_user_error(
    ...
    super_user = db_crud.get_user_by_email(session=db, email=settings.FIRST_SUPERUSER)
    assert super_user
    ...
    assert r.status_code == 403
    assert r.json()["detail"] == "Users cannot delete themselves"
